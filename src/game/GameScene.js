...
export default class GameScene extends Phaser.Scene
{
    ...
    create ()
    {
        this.add.image(400, 300, 'graphics', 'background');
        ...
        this.emojis = this.add.layer();
        const NUM_COLS = 4;
        const NUM_ROWS = 4;
        const CELL_WIDTH = 90;
        const CELL_HEIGHT = 90;
        const X_BASE = (this.scale.gameSize.width - (CELL_WIDTH * NUM_COLS)) / 2;
        const Y_BASE = (this.scale.gameSize.height - (CELL_HEIGHT * NUM_ROWS)) / 2;

        for (let r = 0; r < NUM_ROWS; r += 1) {
          for (let c = 0; c < NUM_COLS; c += 1) {
            const emoji = this.add.image(
              X_BASE + (c * CELL_WIDTH + (CELL_WIDTH / 2)),
              Y_BASE + (r * CELL_HEIGHT + (CELL_HEIGHT / 2)),
              'graphics',
              'smile1',
            );
